...
if (usethreejs && WebGL.isWebGL2Available() && canvasContainer && modelpath) {
  InitScene();
  ...
  if (modelpath) {
    ...
    (async () => {
      ...
      document
        .querySelectorAll(".stickers button")
        .forEach(function (elem, index) {
          if (elem.dataset.id) {
            let choosen = stickerslist.find(
              (sticker) => sticker.id == elem.dataset.id,
            );
...
document.addEventListener("click", async function (e) {
  ...
  // Apply buttons
  if (
    e.target.classList.contains("terror") ||
    ...
    let stattrakval = 0;
    if (stattrak) {
      stattrak.querySelector('input[type="checkbox"]').checked
        ? (stattrakval = 1)
        : (stattrakval = 0);
    }
  ...
  if (
    ...
    viewselect.querySelector("ul").innerHTML = GetHTMLTemplate(
      stickerslist,
      viewlistselected,
    );
...
function InitScene() {
  ...
  hdriLoader.load(
    "https://raw.githubusercontent.com/LielXD/CS2-WeaponPaints-Website/refs/heads/main/src/environment.hdr",
    function (texture) {
    ...
    new GLTFLoader().load(
      ...
      function (weapon) {
        ...
        if (
          currenttype != "knifes" &&
          currenttype != "gloves" &&
          !modelpath.includes("weapon_taser")
        ) {
          if (legacy == true) {
...
async function LoadTexture(texturepath, texturepath_metal) {
  ...
  model.scene.traverse(function (child) {
    if (
      child.isMesh &&
      child.material &&
      !child.material.name.includes("bare_arm") &&
      !child.material.name.includes("scope")
    ) {
...
function getStickerTargetMesh(root) {
  ...
  root.traverse((obj) => {
    ...
    if (matName.includes("scope") || matName.includes("bare_arm")) {
      return;
    }
...
async function ApplySticker(imgsrc = "", place = 0) {
  ...
  if (
    slotOffsets[mainbox.dataset.name] &&
    slotOffsets[mainbox.dataset.name][place]
  ) {
  ...
  const surfacePoint = center
    .clone()
    .add(new THREE.Vector3(offset.x, offset.y, 0));
  ...
  const projPos = surfacePoint
    .clone()
    .add(normalDir.clone().multiplyScalar(distance));
...
function AddMessage(msgContent = "", type = "default", timeout = 3000) {
  if (!msgContent || msgContent == "" || typeof msgContent !== "string") {
    return;
  }
...
viewselect_search.addEventListener("input", function () {
  ...
  if (viewlistcurrent == "stickers") {
    let searched = stickerslist.filter((sticker) =>
      sticker.name
        .toLowerCase()
        .includes(viewselect_search.value.toLocaleLowerCase()),
    );
    viewselect.querySelector("ul").innerHTML = GetHTMLTemplate(
      searched,
      viewlistselected,
    ...
    let searched = keychainslist.filter((keychain) =>
      keychain.name
        .toLowerCase()
        .includes(viewselect_search.value.toLocaleLowerCase()),
    );
...
marks.forEach((elem) => {
  elem.addEventListener("input", function () {
    ...
    if (wear && current["weapon_wear"] != null) {
      let wearoption = wear.querySelector(
        `option[value="${current["weapon_wear"]}"]`,
      );
      if (wearoption) {
    ...
    if (stickerselem) {
      if (current["weapon_sticker_0"]) {
        ...
        if (stickersaved[0] == 0) {
          ...
          let stickerinfo = stickerslist.find(
            (sticker) => sticker.id == stickersaved[0],
          );
    ...
    if (keychains) {
      if (current["weapon_keychain"]) {
        ...
        if (keychainsaved[0] == 0) {
          ...
          let keychaininfo = keychainslist.find(
            (keychain) => keychain.id == keychainsaved[0],
          );
